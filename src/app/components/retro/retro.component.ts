...
export class RetroComponent implements OnInit, AfterViewInit {
  ...
  @ViewChildren(ChildComponentDirective) childComponentHostQueryList: QueryList<
  ...
  ngOnInit() {
    ...
    this.subscription = this.route.params.subscribe(
      params => (self.retroId = params['retroId'])
    )
  ...
  renderPhaseStep() {
    ...
    if (this.childComponentHost && this.childComponentHost.viewContainerRef) {
      this.childComponentService.renderChildComponent(
        this.childComponent,
        this.childComponentHost
      )
    } else {
      this.childComponentHostQueryList.changes.subscribe(
        (comps: QueryList<ChildComponentDirective>) => {
