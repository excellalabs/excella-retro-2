...
export class SubmitFeedbackComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.retroSubscription = this.retroObservable.subscribe(retroVal => {
      ...
      this.currentPhaseSubscription = self.currentPhaseObservable.subscribe(
        currentPhaseVal => {
      ...
      this.existingFeedbackSubscription = self.existingFeedbackObservable.subscribe(
        existingFeedbackVal => {
          self.existingFeedbackVal = existingFeedbackVal
          self.existingFeedbackVal = self.existingFeedbackVal.filter(feedback => {
            return (
              feedback.userId === self.currentUserId && feedback.phaseId === self.phaseId
            )
          })
